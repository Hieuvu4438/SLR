...
for seed in 42 1337 2026; do
  ...
  python -m elsc.provenance --run-dir "$baseline_run" --config "$baseline_config" \
    >>"$log_path" 2>&1
  if [[ "$seed" != 42 ]]; then
    configure_control methods/elsc/configs/ablation_lower_lr.yaml \
...
for seed in 42 1337 2026; do
  ...
  configure_control methods/elsc/configs/ablation_lower_lr_caption.yaml \
  ...
  configure_control methods/elsc/configs/ablation_lower_lr_random_span.yaml \
