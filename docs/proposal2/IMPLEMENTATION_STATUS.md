...
|---|---|---|
| M00 — inventory | passed | `dive doctor` records Python/PyTorch/CUDA and stage resources. RTX 5880 Ada (49,140 MiB), driver 570.195.03, PyTorch 2.11.0+cu128/CUDA 12.8 were detected. How2Sign raw video/pose roots exist and both official SEDS Baidu shares are live. Pinned SEDS source and its checked native reproduction contract are present; SEDS initial/locked checkpoints remain unavailable because current Baidu routes these >50 MB anonymous downloads through its authenticated desktop-client flow. |
...
| M04 — data contracts | passed | Versioned manifest/split guards, normalized word/numeric units, native subword offset mapping with partial-target rejection, deterministic canonical/shifted grids, raw-time RF mapping, per-step pose normalization, P/C construction and bool-mask collation have independent fixtures. The current real `dive prepare-data` run contains 34,888 controlled How2Sign samples with frame-exact stored video/pose maps, disjoint sources, multi-video positives and 23,026 ambiguity exclusions after exact native SEDS text normalization. The native validation path additionally captures exact non-identity raw indices after SEDS subsampling/hand filtering, checks finite RGB schema and RGB/pose clip-count equality, and binds the tokenizer. `dive validate-data` enforces exact bidirectional relevance coverage, legal excluded negatives, source-video split isolation, media/pose/RGB-key/frame-map presence and translation provenance, then registers checksummed audit/lineage outputs plus isolated train-only manifest/relevance/exclusion snapshots so mining never depends on a mixed-split directory; real execution currently stops before this native pass at the absent SEDS I3D cache as intended. |
| M05 — SEDS adapter | in_progress | Code is complete for clean pinned-checkout/model-state validation, a source-hashed and runtime-typed native How2Sign flag contract, explicit missing-parser compatibility defaults, a manifest-driven bridge through pinned pose/RGB/text/collate methods, lazy official construction, native contextual encoding, prelogit fusion directional mixing, mask-before-softmax, explicit CLS/text-mask conversion, projected subword unit pooling, pre-Transformer RGB tap, cloned local GCN/sign-conv pose path and conservative raw RF metadata. `dive baseline validate` now runs a real released-dispatcher unpadded parity gate and full controlled-dev gallery when assets exist. Native input/scorer fixtures verify shapes, masks, IDs, RNG isolation and clean-checkout preservation; compatible real checkpoint/features remain unresolved. |
| M06 — B0 reproduction | blocked | The released SEDS loader has no dev annotation and selects training checkpoints on `test.pkl`; this is now isolated as published/debug only. Controlled manifests use 31,019 pinned-upstream train videos, an independent 1,527-sample dev split, and reserve all 2,342 pinned-upstream test videos for final evaluation, with zero source overlap. The controlled native 200-epoch trainer, rolling resume state, earliest-best dev selection, full-gallery validation and provenance outputs are implemented. Real execution still requires the official I3D, CLIP and SignBERT assets. Existing `H2S_sota.pth` is CiCo and is not relabeled as SEDS. |
| M07 — evidence/reference | in_progress | Code is complete for the pointwise local RGB+pose projector, frozen BN mode/affine, audited AdamW ownership/decay groups, fixed-step warm-up/cosine schedule, symmetric `E_local` retrieval-only training, full-dev teacher evaluation, earliest-tie checkpoint selection, checksummed reference export, deep-copy independence and identity initialization. `dive evidence warmup` now resolves and rehashes every controlled parent, revalidates feature/token lineage, converts native scalar clip starts into explicit half-open local windows, caches frozen contextual units, streams native video batches, evaluates the exact full gallery in bounded blocks, resumes model/optimizer/scheduler/RNG/history, and registers the selected FP32 reference. A real warm-up/reference artifact remains blocked on the upstream assets and preceding controlled B0 run. |
| M08 — mining/audit | blocked | Code is complete for bidirectional blockwise pooled shortlist, sparse exact S0 four-margin reranking, deterministic dedup/ties, mutually eligible cross-pair filtering, fixed-anchor full-gallery shortlist coverage, strict numeric schema, blinded audit export, train-only finalize and fingerprint/checksum bank I/O. `dive mine --phase propose` consumes only registered train caches/relations, rejects partial native unit targets, resumes checksummed score chunks, registers candidate/rejection/coverage artifacts and never reads student or test data. `dive audit export` supplies sign-video context without student outputs; its decision loader requires checksummed completed ratings, reviewer identity/time and explicit status. `dive mine --phase finalize` refuses pending/rejected audits and binds accepted human evidence, B0/reference/timestamps and unit lineage into an immutable pre-support bank while preserving all rejections. A real accepted human audit plus the blocked SEDS cache run are required to produce the main semantic bank. |
...
| M11 — end-to-end smoke | in_progress | `dive smoke` runs a deterministic CPU fixture through student forward, H=2/H_active=1 losses, optimizer update, checkpoint+checksum, reload parity and ID-based evaluation while emitting `benchmark_claim_allowed=false`. The run-state resolver now enforces checksummed shared/per-variant namespaces, immutable stage records, parent revalidation and no filesystem discovery. `dive cache build --kind frozen_train` now writes five provenance-bound, sample-addressed caches with FP32 reference features and raw-time RFs; one-pass writers resume validated atomic shards and publish indexes only at complete manifest coverage. Tiny real SEDS integration remains blocked by checkpoint/features. |
...
- The official SEDS model share was verified to list the 804,176,411-byte How2Sign checkpoint and
  ...
  credentials. The latest filesystem check reports 165 GiB free, enough headroom for the listed
  downloads and projected frozen caches; extraction remains gated on acquiring the official files
  and checking their actual expanded size.
...
- The checked SEDS reproduction artifact validates the exact pinned train/eval script SHA-256s,
  published flag names/values/types, construction fields and controlled split policy. Real
  `validate_data` diagnostics rehash every prepared parent and now name
  `third_party/SEDS/How2Sign/I3D_features` as the sole missing validation resource; baseline
  diagnostics additionally name the concrete released checkpoint path.
- `pytest -q methods/dive/tests`: 110 passed after the checked reproduction/diagnostic increment;
  `pytest -q`: 234 passed with no cross-method regressions.
- `pytest -q methods/dive/tests`: 112 passed after the controlled-manifest/native-SEDS input bridge;
  `pytest -q`: 236 passed. Its integration fixture invokes the pinned tokenizer, pose transforms,
  RGB loader and collate path while confirming the pinned checkout remains clean.
- `dive baseline validate --config methods/dive/configs/how2sign_base.yaml --split dev` exits 2 at
  the required unregistered `validate_data.audit`; its doctor report independently names that
  parent, the missing I3D directory and released checkpoint. The command is otherwise wired through
  native encoding, released-dispatcher FP32 parity, full-gallery scoring, ID-based evaluation and
  run-state registration. `pytest -q methods/dive/tests`: 114 passed; `pytest -q`: 238 passed.
- Native SEDS preprocessing instrumentation proves that its model timeline is not always the stored
  pose timeline: tests capture deterministic `320 -> 160` frame subsampling and explicit removal of
  a frame where both hand crops fail, then verify RF intervals traverse those raw indices.
  `pytest -q methods/dive/tests`: 117 passed; `pytest -q`: 241 passed.
- The How2Sign data validator now executes the pinned native loader across all routed samples once
  I3D files exist, validates finite RGB layout and exact pose/RGB clip counts, binds the tokenizer,
  and registers per-split selected-frame/clip-start lineage. Baseline validation consumes and
  rechecks that artifact as a required parent. Fixture wiring and failure contracts pass;
  `pytest -q methods/dive/tests`: 118 passed; `pytest -q`: 242 passed.
- Exact native SEDS text normalization regenerated the registered controlled manifests at
  31,019/1,527/2,342 train/dev/test videos and 30,786/1,527/1,964 unique texts, with 23,026
  registered ambiguity exclusions and zero model-normalization mismatches across all samples.
- `dive evidence warmup` now has a native, provenance-locked train/dev runner. Dense-versus-blocked
  gallery scoring, variable-length frozen-unit collation, excluded-positive rejection, immutable
  unit-lineage loading, CLI parent gates and interrupted schedule/history resume have fixture
  oracles. `pytest -q`: 258 passed; Ruff and whitespace checks pass. Real execution stops at the
  already recorded missing I3D/model asset gate rather than producing a synthetic reference.
- The frozen-train cache command now exports native video/text, local RGB, contextual unit and
  frozen-reference tensors as separately fingerprinted shards. Tests cover one-pass publication,
  interruption/resume, reopened-index validation, safe reference loading, parent fingerprint
  rejection, multi-view sample addressing and native scalar-start to half-open-window conversion.
...
- The executable proposal runner now binds prepared relevance/exclusions, validated unit lineage and
  native cache order; filters strict-numeric candidates before audit; records every semantic
  rejection; and recovers an atomically renamed but not-yet-registered result only after validating
  proposals, shortlists, coverage and pair-score checksums. A registered synthetic integration
  exercises build/recovery/idempotence; `pytest -q`: 279 passed. The real doctor reports only the
  still-missing validation/cache parents, and the real command exits before creating mining output.
- Human-audit export and bank finalization are now executable, with immutable annotation content,
  explicit accepted/rejected decisions, UTC reviewer provenance, audit snapshots, strict train-only
  inputs and crash-recovery validation for bank/rejection/checksum outputs. Validation also registers
  isolated train snapshots instead of routing mining through a mixed-split relevance directory.
  The accepted-audit integration reaches a registered pre-support bank; `pytest -q`: 283 passed.
  Real audit/finalize commands stop at the missing proposal/cache/reference parents as intended.
