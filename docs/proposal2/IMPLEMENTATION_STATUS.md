...
| M08 — mining/audit | blocked | Code is complete for bidirectional blockwise pooled shortlist, sparse exact S0 four-margin reranking, deterministic dedup/ties, mutually eligible cross-pair filtering, fixed-anchor full-gallery shortlist coverage, strict numeric schema, blinded audit export, train-only finalize and fingerprint/checksum bank I/O. `dive mine --phase propose` now consumes only registered train caches/relations, rejects partial native unit targets, resumes checksummed score chunks, registers candidate/rejection/coverage artifacts and never reads student or test data. A real accepted human audit plus the blocked SEDS cache run are required to produce the main semantic bank. |
