...
Not completed: full raw-data/checkpoint downloads and hash/load verification; complete CMCM/GTRN full-text audit; neural baseline reproduction; GPU OCEM implementation; SLRet training/evaluation. Next phase is the resource/reproduction gate, followed by measuring the hypothesized failure. Do not claim that mathematical scorer checks passed the reproduction gate.
...
**Measured H2S annotation snapshot:** downloaded official train/dev files contain 31,165/1,741 rows with unique sample IDs. Train: 30,109 raw captions; 1,509 rows in repeated-caption classes; 1,056 duplicate excess (1,063 after case/whitespace normalization). Dev: 1,516 raw captions; 444 rows in repeated classes; 225 excess. Forty-seven dev captions occur verbatim in train; train/dev `VIDEO_ID` intersection is zero. No blank captions or nonpositive durations. These are annotation counts, not canonical usable-video counts or proof against visual leakage. Test TSV returned quota HTML.
...
**F6:** A SEDS-independent route exists at endpoint level. **Evidence:** raw P14T/H2S and Oxford I3D/CLIP/detector binary ranges. **Implication:** implementation is plausible, but full integrity, loading, manifests and H2S test annotations remain gates.
...
A successful byte range is **not** full download, hash or model-load verification. All large artifacts retain an integrity gate.
...
| H2S test annotations | ID `1AgwBZW26kFHS4CWNMQTCMPGkBPkH3qCu`: HTTP200 quota HTML, not TSV | Critical gate; retry official route or later author release. No quota bypass/unverifiable mirror |
| Oxford I3D `bsl5k.pth.tar` | HTTP206, 142,594,302 bytes: [Oxford](https://www.robots.ox.ac.uk/~vgg/research/bslattend/data/bsl5k.pth.tar) | Required baseline initialization. Full hash/load pending; a different generic I3D changes comparability |
| CLIP `openai/clip-vit-base-patch32` | Official Hugging Face config and weight range; 605,247,071 bytes | Required; pin revision/hash and architecture mapping. Official OpenAI implementation is a fallback requiring equivalence checks |
...
**CRITICAL:** Can full P14T/Oxford/CLIP artifacts be verified and loaded? Can the official H2S test annotations and exact 2,348 manifest be obtained? Can CiCo reproduce both R@1 directions within 1pp on matching IDs? Are baseline false matches more concentrated after length/lexical controls? Does this persist in the pre-contextual local head? Does real A beat ordinary OT/shuffled A? Does CMCM invalidate novelty or change the strongest fair target?
...
2. **Gate 0:** obtain full official P14T/Oxford I3D/CLIP artifacts, hashes and load checks. Freeze environment, repository commits, crops, timestamps, tokenizer and sample IDs. Resolve H2S test TSV/exact manifest before claiming cross-dataset reproducibility. No unofficial feature archives.
