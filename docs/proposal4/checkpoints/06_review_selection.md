...
Literature cutoff date: 10 September 2026. Checkpoint 6: reviewer simulation and conditional selection.
...
Completed: multi-query seed search; primary full-text reading of SPOT-ALIGN, CiCo, UPRet, SEDS, C²RL, SAN, CSLR² and Scaling up Multimodal Pre-training; publisher/author verification of CMCM and GTRN. Literature review is bounded, not an assertion of exhaustive coverage. CMCM full text and GTRN full PDF remain inaccessible. Dataset/protocol/resource audit completed with explicit access limitations. Completed: failure taxonomy and gap ranking. Completed: exactly five candidates and provisional scoring. Completed: bounded adversarial novelty audit. Completed: three simulated reviewer perspectives, revisions and selection of exactly one conditional method, OCEM. Current: full mathematical specification, experiment ladder and A–Z report. No SLRet model has been trained here.
...
Novelty decision: generic partial/null OT is REJECTED. A physical-support capacity formulation survives only as a narrow testable hypothesis. Do not revive candidate 4 as “novel graph transport” or candidate 5 as “new false-negative removal”: adjacent prior art directly limits those claims.
...
Selected for a gated pilot: **Overlap-Constrained Evidence Matching (OCEM)**. Core claim is conditional: concentrated, correlated support may inflate near-miss retrieval scores. Preserve the independently reproduced CiCo branch. Add a 1024→512 linear local projection on frozen, pre-contextual I3D window features; reuse contextual text tokens. Actual local windows define intervals. Partition their union into disjoint atoms. A_ai=|atom_a∩window_i|/|window_i|; w_a=|atom_a|/|union|. Align text token mass b_j=1/m to real windows plus a null state. Maximize <P_real,C>−epsilon KL(P||R), subject to text row sums b and A P_real^T 1 <= kappa w. Reference R=b⊗[(1−pi)q,pi]; q splits canonical support mass among exact duplicates. Use centered optimal value phi(C)=F(C)−F(0). Scores per direction are (1−alpha)S_CiCo+alpha phi. Base CLCL plus a sampled contrastive loss on this SAME mixed score forms the objective. Random plus real gallery hard negatives; exact duplicate captions masked identically for all matched controls. No synthetic captions or new pose/gloss/LLM supervision. No monotonic text/sign alignment. No claim that capacity is a linguistic law.

Reviewer A (sign-language expert): 4/10, confidence 4/5. Reject a one-word/one-sign or duration-equals-information interpretation; contextual tokens have no local physical support; morphology and simultaneous non-manual information permit legitimate evidence reuse. Revisions: local-only score input; soft mass/null, loose capacities; preserve contextual branch; diagnose harms on compression/length; no linguistic-boundary claims.

Reviewer B (retrieval expert): 4/10, confidence 4/5. Generic OT/coverage is old; compare uniform/duration-weighted partial OT, independent clip caps, and plain local matching at matched parameters. Whole-gallery latency and both directions matter. Revisions: narrow novelty to shared-support constraint; no new solver claim; same score at train/test; exact full-gallery primary evaluation; ordinary partial OT is a mandatory falsification baseline.

Reviewer C (hostile A*): 3/10, confidence 5/5. No observed mechanism, no baseline reproduction, no verified CMCM comparison, and only synthetic solver checks. Revisions: no claimed SOTA; baseline ±1-point gate; real validation Stage A; cross-dataset seeds and paired confidence intervals; kill if duplicate masking or ordinary OT explains gain.

Scores assess a hypothetical submission with CURRENT evidence; they are not actual independent reviews and are not raised merely because the prose was revised. The revisions make a pilot defensible, not a paper acceptable.

C1 remains highest after reconsidering C2–C5. Decision utility 60.4/100 is not probability. Present probability of beating the strongest matched fair baseline on bidirectional R@1 across two datasets: **30–50%**, an explicitly subjective band with no real pilot evidence. The request for 70–85% cannot be met honestly. Conditional continuation is limited to cheap validation; scale only after the specified gates. No inaccessible SEDS resource is required.

### 12. Novelty audit state

Queries completed: sign retrieval + coverage/transport/evidence; sign translation + capacity/partial OT; video-text retrieval + overlap/overcounting/partial OT/receptive field; multimodal retrieval + shared evidence/redundancy/OT; capacity-constrained OT; neural translation + coverage. No exact implementation of the proposed raw-support inequality was located. Empty or irrelevant query results are weak evidence, not proof of novelty.

Closest collisions examined in primary full texts: UPRet already has SLRet OT; VTaMo uses a null token with uniform marginals; DualAnchor (2607.27614v1, full PDF now downloaded) uses partial transport with dustbins and fixed genuine mass, removed at inference; its “retrieval-quality” analysis is lexical-generation diagnostics stratified by cosine similarity, not standard SLRet R@K. AVIOT (2608.20473v1, full PDF downloaded) uses balanced source-to-compact-support transport for VideoLM compression and question-conditioned temporal allocation. Neither inspected formulation contains the proposed overlapping receptive-field load inequality. Korman, McCann & Seis (1307.7774) establish capacity-constrained OT prior art; Tu et al. (1601.04811) already use coverage to prevent over/under-translation. Norton, Drop-DTW, ColBERT, SignCL and CrossCLR further narrow claims.

Revision: Do NOT claim new OT, partial matching, coverage, late interaction, a new convex solver, or arbitrary speed invariance. Claim only a support-aware retrieval scoring hypothesis, tested on densely overlapping local sign features. Use pre-contextual I3D features so an interval has a real receptive field; contextual CiCo tokens retain the baseline role. Null handles unmatched textual mass without a one-word/one-sign rule. Exact duplicated-token invariance holds only when copies have identical features/supports and share their reference mass. A count-balanced OT implementation with similarly split marginals can share this property; it is not unique evidence of novelty.

A NumPy/SciPy reference implementation is at output/ocem_reference.py. Synthetic feasibility, exact-duplicate invariance, text-row permutation, and finite-difference envelope checks pass. These are mathematical checks, NOT a dataset pilot. Capacity is a modeling prior, not a proven conservation law of sign semantics. Full CMCM paper remains an unresolved prior-art/SOTA risk.
...
New reproducibility detail: CiCo paper describes BSL-1K pretraining, while its official README/extractor points to Oxford bsl5k.pth.tar. Preserve that distinction; pin the actual artifact and classify any mismatch during reproduction. The PHOENIX evaluation command uses alpha=0.9, unlike H2S/CSL alpha=0.8. The published CLCL loss applies symmetric InfoNCE to BOTH directional aggregation matrices; preserve this in reproduction. Crop-script dependencies include per-sample bounding-box files; regenerate with a pinned public detector if needed and classify changed crops rather than silently claiming exact reproduction.
...
1. Finish the A–Z report, complete OCEM equations/pseudocode and specify fair full-gallery experiments and numerical decision gates.
2. Keep the newly discovered metrics tie issue diagnostic, not the paper contribution.
3. Preserve the empirical kill criterion: no validation error association or no advantage over ordinary partial OT means abandon the OCEM thesis.
...
6. Persist the final report, reference scorer, mathematical-check results, measured resource/annotation audit and a compact final checkpoint.
