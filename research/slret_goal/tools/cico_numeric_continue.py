"""Matched PH/CSL CiCo numerical replication; original PH runner preserved as v1."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument('--dataset',choices=['ph','csl'],default='ph')
    parser.add_argument('--seed',type=int,choices=[42,1337,2026],default=42)
    ...
    report=dict(run_id=cli.run_id,status='running',pid=os.getpid(),command=sys.argv,arm=cli.arm,
        smoke=cli.smoke,test_loaded=False,dev_loaded=not cli.smoke,seed=cli.seed,dataset=cli.dataset,batch_size=512,
    ...
    try:
        ...
        sources=['methods/elsc/elsc/train.py','methods/elsc/elsc/evaluate.py','methods/elsc/elsc/losses/coarse.py',
                 ...
                 'shared/slr_common/data/tokenize.py','shared/slr_common/data/group_sampler.py',
                 'third_party/SLRT/CiCo/CLCL/modules/modeling.py',
                 'research/slret_goal/tools/cico_fp32_moments.py']
        report['source_sha256']={p:sha(ROOT/p) for p in sources}
        data=CiCoFeatureDataset(ROOT/cfg['data']['train_manifest'],feature_len=64,alpha=cfg['data']['alpha'],split='train')
        collator=CiCoCollator(load_cico_tokenizer(cfg),32,augment=True,seed=cli.seed)
        generator=torch.Generator().manual_seed(cli.seed)
        grouped=CaptionGroupSampler(data.records,seed=cli.seed) if cfg['train'].get('one_video_per_caption_group',False) else None
        loader=torch.utils.data.DataLoader(data,batch_size=512,shuffle=grouped is None,sampler=grouped,generator=generator,
            drop_last=True,num_workers=8,pin_memory=True,collate_fn=collator)
        expected_steps=13 if cli.dataset=='ph' else 12
        assert len(loader)==expected_steps
        scheduler=_scheduler(optimizer,len(loader)*200,.1)
        report['schedule_total_steps']=len(loader)*200
        report['steps_per_epoch']=len(loader)
        report['manifests']={s:sha(ROOT/cfg['data'][f'{s}_manifest']) for s in ['train','dev']}
        if not cli.smoke:
            devdata=CiCoFeatureDataset(ROOT/cfg['data']['dev_manifest'],feature_len=64,alpha=cfg['data']['alpha'],split='dev')
        ...
        for epoch in range(1 if cli.smoke else 20):
            ...
            if grouped is not None:grouped.set_epoch(epoch)
            for batch in loader:
                started=time.time()
                model.train()
