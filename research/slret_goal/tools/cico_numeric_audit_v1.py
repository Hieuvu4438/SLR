...
def check_rows(a,b):
    if len(a)!=260 or len(b)!=260:raise ValueError('260stepsrequired')
    for index,(x,y) in enumerate(zip(a,b)):
        if x['step']!=index+1 or x['epoch']!=index//13 or len(x['ids'])!=512:
...
def main():
    ...
    try:
        ...
        keys=['initial_checkpoint_sha256','config','source_sha256','script_sha256','protocol_sha256','manifests','initial_model_digest',
        ...
        assert runs[0]['optimizer_updates']==260 and runs[0]['examples_seen']==133120
        ...
        report.update(paired_steps=260,examples_per_arm=133120,verified_feature_files=len(features))
        ...
        texts=[r.caption_id for r in records]
        assert len(vids)==len(set(vids))==len(set(texts))==519
        v2t={r.video_id:[r.caption_id] for r in records}
        t2v={r.caption_id:[r.video_id] for r in records}
        report['score_hashes']={}
        for root,run in zip(roots,runs):
            assert set(map(int,run['evaluations']))==set(range(0,261,13))
            assert sha(root/'feature_hashes.json')==run['feature_hashes_sha256']
            for step in range(0,261,13):
        ...
        endpoint={d:runs[1]['evaluations']['260'][d]['R1']-runs[0]['evaluations']['260'][d]['R1'] for d in ['T2V','V2T']}
