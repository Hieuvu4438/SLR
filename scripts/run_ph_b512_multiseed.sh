...
run_seed() {
  ...
  configure_stage configs/ph_min.yaml "$baseline_run" "$cache_path" "$min_config"
  check_disk
  ...
  for control_name in adapter_only caption random_span; do
    case "$control_name" in
      adapter_only) template=configs/ablation_adapter_only.yaml ;;
      caption) template=configs/ablation_caption.yaml ;;
      random_span) template=configs/ablation_random_span.yaml ;;
    esac
