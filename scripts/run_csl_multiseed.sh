...
while [[ ! -f "$seed42_gate" ]]; do
  ...
  if ! pgrep -f -- "bash scripts/run_csl_gate_x.sh" >/dev/null; then
...
run_controls() {
  ...
  for control_name in adapter_only caption random_span; do
    case "$control_name" in
      adapter_only) template=configs/csl_adapter_only.yaml ;;
      caption) template=configs/csl_caption.yaml ;;
      random_span) template=configs/csl_random_span.yaml ;;
    esac
...
run_seed() {
  ...
  configure_stage configs/csl_min.yaml "$baseline_run" "$cache_path" "$min_config"
  check_disk
